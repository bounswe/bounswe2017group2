...
urlpatterns = [
    ...
    url(r'^auth/token/obtain/$', TokenObtainPairView.as_view()), # obtains a new JWT Access Token
    url(r'^auth/token/refresh/$', TokenRefreshView.as_view()), # refreshes the JWT Access Token
    url(r'^auth/token/verify/$', TokenVerifyView.as_view()), # verifies a JWT Access Token
    # USER
    ...
    url(r'^user/me/$', views.get_user_info), # returns the logged in user object
    url(r'^user/(?P<pk>[0-9]+)/follow/$', views.follow_user), # the logged in user follows the one with given pk
    url(r'^user/(?P<pk>[0-9]+)/unfollow/$', views.unfollow_user), # the logged in user unfollows the one with given pk
    url(r'^user/get_concerts/$',views.get_user_concerts), # returns all the concerts of the logged in user
    url(r'^user/deactivate/$', views.deactivate_user, name='deactivate_user'), # deactivates the account of the logged in user
    ...
    # CONCERT
    url(r'^concert/(?P<pk>[0-9]+)/subscribe/$', views.subscribe_concert), # subscribes logged in user to the concert
    ...
    # COMMENT
    url(r'^concert/(?P<pk>[0-9]+)/newcomment/$', views.create_comment), # adds a new comment by the logged in user to the concert specified by its primary key
    # RATING
    url(r'^concert/(?P<pk>[0-9]+)/rate/$', views.rate_concert), # adds a new rating by the logged in user to the concert specified by its primary key
    # LOCATION
    ...
    # TAG
    url(r'^tags/(?P<search_str>[\w\-]+)/$',views.get_tags) # returns a list of tags by doing a search on Wikidata
    # REPORT
